...
Optional (empfohlen) zusätzlich:
- `DATABASE_URL`

Beispiel (Platzhalter, konsistent):
...
## Häufiger Produktionsfehler (28P01)
- Wenn `password authentication failed for user "kauvio"` erscheint, stimmen `POSTGRES_PASSWORD` und `DATABASE_URL` nicht überein **oder** das Postgres-Volume enthält noch ein altes Passwort.
- Dann: Stack stoppen → Postgres-Volume löschen → mit konsistenten ENV-Werten neu deployen.
