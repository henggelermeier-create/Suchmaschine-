...
function galleryFor(item = {}) { return [...new Set([imageFor(item), ...((item.image_gallery || item.images || item.gallery || []).filter(Boolean))].filter(Boolean))].slice(0, 12) }
...
function ProductGallery({ product }) {
  ...
  useEffect(() => { setSelected(images[0] || null) }, [product?.slug, images[0]])
  if (!images.length) return <div className="product-gallery-empty"><ImageOff size={34} /><span>Kein Produktbild</span></div>
  return <div className="product-gallery"><div className="product-gallery-main"><img src={selected || images[0]} alt={product.title || 'Produkt'} /></div>{images.length > 1 ? <div className="product-gallery-thumbs">{images.map((src, index) => <button key={`${src}-${index}`} className={src === selected ? 'active' : ''} onClick={() => setSelected(src)}><img src={src} alt={`Bild ${index + 1}`} /></button>)}</div> : null}</div>
}
...
function ProductPage({ slug }) {
  ...
  return <main className="page-shell product-page-shell youth-page"><section className="hero-panel product-hero-modern product-hero-gallery"><div className="product-hero-copy"><span className={`chip ${toneFromLabel(product.decision?.label || product.deal_label, product.offer_count || 0)}`}><Bot size={14} /> {product.decision?.label || product.deal_label || 'KI Vergleich'}</span><h1 className="product-title">{product.title}</h1><p className="hero-text">{product.ai_summary || 'KI aufbereiteter Produktvergleich für die Schweiz.'}</p><div className="status-row top-margin-sm"><span className="status-chip tone-hot"><Trophy size={14} /> {formatPrice(product.price)}</span><span className="status-chip tone-mint"><Store size={14} /> {product.shop_name || 'KI Index'}</span><span className="status-chip tone-violet"><Bot size={14} /> {product.offer_count || 0} Angebote</span></div></div><ProductGallery product={product} /></section><section className="card-panel glass-panel"><div className="section-head"><div><h2>Preisvergleich</h2><p className="muted no-margin">Nur aktive Produkt-Angebote. Gelöschte Einträge erscheinen nicht mehr.</p></div></div><div className="offer-list">{(product.offers || []).map((offer, index) => <div className="offer-row modern-offer-row" key={`${offer.id || offer.shop_name}-${index}`}><ProductImage item={offer} compact /><div className="offer-main"><div className="offer-shop">{offer.shop_name}</div><div className="muted small">Zuletzt aktualisiert: {formatDate(offer.updated_at)}</div></div><div className="offer-row-right"><strong className="price-inline">{formatPrice(offer.price)}</strong><a className="btn btn-primary btn-small" href={`/r/${product.slug}/${encodeURIComponent(offer.shop_name)}`} target="_blank" rel="noreferrer">Zum Shop</a></div></div>)}</div></section><SuggestionPills items={product.similarItems || []} title="Ähnliche Produkte" /><SuggestionPills items={product.suggestions || []} title="Weitere Vorschläge" /></main>
}
...
function AdminPage() {
  ...
  const brainStats = [{ label: 'KI Modus', value: 'Auto', icon: BrainCircuit, tone: 'tone-mint' }, { label: 'DB Produkte', value: numberValue(systemHealth?.checks?.canonical_products?.count), icon: Database, tone: 'tone-mint' }, { label: 'Angebote Cache', value: numberValue(systemHealth?.checks?.source_offers_v2?.count), icon: Zap, tone: 'tone-violet' }, { label: 'Shop Quellen', value: numberValue(systemHealth?.checks?.swiss_sources?.count), icon: Store, tone: 'tone-violet' }, { label: 'KI Seeds', value: numberValue(dashboard?.stats?.autonomousSeeds), icon: Sparkles, tone: 'tone-hot' }, { label: 'Suchjobs', value: numberValue(dashboard?.stats?.searchTasks), icon: Activity, tone: 'tone-violet' }]
